...
def create_register_participant_button(window: Frame, self) -> None:
    button = Button(
        master=window, text=REGISTER_BUTTON_TITLE,
        width=REGISTER_BUTTON_SIZE, fg=BUTTON_TEXT_COLOR,
        command=lambda: register_new_participant(self=self)
    )
    button.grid(**REGISTER_BUTTON_COORDS)


def create_unregister_participant_button(window: Frame, self) -> None:
    button = Button(
        master=window, text=UNREGISTER_BUTTON_TITLE,
        width=UNREGISTER_BUTTON_SIZE, fg=BUTTON_TEXT_COLOR,
        command=lambda: unregister_participant(self=self)
    )
...
def create_open_edit_category_toplevel_button(self, window: Frame) -> None:
    button = Button(
        ...
        command=lambda: clicked_open_edit_category_toplevel(self=self)
    )
